...
(** Appends elements of an OCaml list to the front of a Lisp list (Value.t).
    list1 is ('a list), list2 is the Value.t representing the rest of the list.
    Returns a new Value.t list structure. *)
let rec list_append (list1 : t list) (list2 : t) : t =
  match list1 with
  | [] -> list2 (* Base case: return the second list *)
  | h :: t ->
      (* Create a new cons cell (mutable fields) *)
      Cons { car = h; cdr = list_append t list2 }